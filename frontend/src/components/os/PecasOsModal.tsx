    ...
    <div className="rounded-xl border border-border/70 bg-gradient-to-br from-white to-slate-50/70 p-3 shadow-sm transition-all hover:border-cyan-200 hover:shadow-md dark:from-slate-900 dark:to-slate-950 dark:hover:border-cyan-400/30">
      <div className="grid gap-3 lg:grid-cols-[1.2fr_1.4fr_100px_140px_140px_42px]">
        <div className="space-y-2">
          ...
          <Input
            value={item.servicoNome || 'Produto sem nome'}
            readOnly
...
export function PecasOsModal({
  ...
  const totalPecas = pecas.reduce((acc, item) => acc + (Number(item.quantidade) * Number(item.valorUnitario)), 0)

  return (
    <Dialog open={open} onClose={onClose} title="Peças e Produtos" description="Itens físicos usados na OS, preparados para baixa de estoque." contentClassName="max-h-[85vh] max-w-5xl overflow-y-auto bg-slate-50/50 dark:bg-slate-950/50">
      <div className="pb-6">
        <div className="mb-4 flex items-center justify-between rounded-xl border border-cyan-200/50 bg-cyan-50/30 p-4 dark:border-cyan-400/20 dark:bg-cyan-900/10">
          <div>
            <h4 className="font-semibold text-foreground">Total em Peças/Produtos</h4>
            <p className="text-sm text-muted-foreground">{pecas.length} item(ns) registrado(s)</p>
          </div>
          <div className="flex items-center gap-4">
            <span className="text-xl font-bold tracking-tight text-foreground">{formatCurrency(totalPecas)}</span>
        ...
        <div className="space-y-3">
          {pecas.length === 0 ? (
            <p className="rounded-lg border border-border bg-muted/30 p-6 text-center text-sm text-muted-foreground">
              Nenhuma peça ou produto adicionado até o momento.
            </p>
          ) : (
            pecas.map((item) => (
              <PureProdutoRow
